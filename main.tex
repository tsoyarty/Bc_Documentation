...
theses. I utilized artificial intelligence for grammar correction.
\medskip
\\\\
...
Figure \ref{fig:maze_clutter} presents a cluttered map where all methods exhibit similar behavior. 
...
\begin{figure}
  ...
  \end{subfigure}  
  \caption{Difficult map with numerouse narrow passages \ref{fig:maze_clutter_Cspace} 
...
completing 5000 iterations in $\smallsim$30 seconds, while RRT*\_ML took $\smallsim$1041 seconds 
...
\begin{figure}[!ht]
  ...
  \begin{subfigure}[t]{0.49\textwidth}
    ...
    \caption{Time taken to learn predicting points (Algorithm \ref{alg:ml_sample}) 
    in the 3D workspace with the 6D $\mathcal{C}$-space (\ref{fig:6Dconfig-space})
    to be in the \(C_{\text{free}}\).}
