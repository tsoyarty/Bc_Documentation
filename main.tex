...
\begin{thanks}
I am sincerely thankful to my supervisor Ing. Vojtěch Vonásek, Ph.D., 
for his assistance and invaluable feedback. 
His guidance has been instrumental in navigating 
the challenges and refining the outcomes of this research endeavor. 
Additionally, I am truly grateful for the opportunity to study at 
CTU, where I have gained knowledge and skills 
that have greatly contributed to the successful completion of 
this thesis. 
I would also like to express my gratitude to my family and 
...
theses. 
Furthermore, I utilized artificial intelligence for grammar correction.
\medskip
\\\\
...
It represents all possible configurations or states that a robot or system can occupy within its environment.
%---------------------------------------------------------------------------------
\begin{figure}[!ht]
  \centering
  ...
  \begin{subfigure}[b]{0.36\textwidth}
      \includegraphics[width=\textwidth]{figChap1/RRTsimple2_cropped.pdf}
  ...
  along with a representation of the Rapidly-exploring Random Trees (RRT) algorithm in Figure \ref{fig:RRTsimple}.}%\label{fig:animals}
\end{figure}
%---------------------------------------------------------------------------------
\\
...
In Figure \ref{fig:RRTsimple}, simple examples of how RRT looks can be seen.
%---------------------------------------------------------------------------------
\begin{figure}[!ht]
  \centering 
  \begin{minipage}[t]{0.9\textwidth}
      \centering
      \includegraphics[width=0.49\textwidth]{figChap1/narrow_passage_cropped.pdf} 
  \end{minipage} 
...
requiring the algorithm to derive insights from the data independently.
%---------------------------------------------------------------------------------
\begin{figure}[!ht]
  \centering
...
%------
Our objective is to find a sequence of configurations that safely moves the piano (our \textbf{robot})
...
\begin{figure}
  ...
  ~ %add desired spacing between images, e. g. ~, \quad, \qquad, \hfill etc. 
    %(or a blank line to force the subfigure onto a new line)
  \begin{subfigure}[b]{0.4\textwidth}
      \includegraphics[width=\textwidth]{figChap1/narrow_passage_cropped.pdf}
      \caption{Narrow passage in the 2D workspace with a 3D $\mathcal{C}$-space.}
      \label{fig:narrowpassage2}
  \end{subfigure} 
  \caption{llustration of the A* algorithm realizes path-finding on grid map in Figure \ref{fig:Astargrid} and 
  illustration of the narrow passage in Figure \ref{fig:narrowpassage2}, 
  where the blue lines represent RRT* exploration in the workspace.
...
%------
In Figure \ref{fig:config_space}, a simple  2D workspace is depicted, 
wherein the robot is represented by a point.
A workspace is the area within which a robot operates, 
and in this context, the configuration of the robot 
can be described by two coordinates, such as $q=(x,y)$.
Therefore, it has a 2D $\mathcal{C}$-space. 
...
The black circle at the center of the space represents the $\mathcal{C}_\text{obs}$. 
The red ring represents the boundary of the $\mathcal{C}$-space, 
with points inside it corresponding to the $\mathcal{C}_\text{goal}$.
...
\begin{figure}[!ht]
  ...
  The green circle denotes the initial configuration (\( q_{\text{start}} \)), 
  while the yellow circle represents the new node to be added (\( q_{\text{new}} \)), 
  along with its transparent yellow circular area with certain radius (\( Q_{\text{near}} \)). 
  Figure \ref{fig:nearest_nodes} displays the nodes within this circular area. 
  A parent node for the new node, identified by the red circle (\( q_{\text{min}} \)), 
...
\begin{figure}[!ht]
  ...
  Comparison in a 3D workspace with a 6D $\mathcal{C}$-space, with the robot represented 
  as a purple cube and the obstacle 
...
\begin{figure}[!ht]
  ...
  \end{subfigure}
  \caption{
    In the 2D workspace depicted in Figure \ref{fig:prm_maze}, 
    with a corresponding 2D $\mathcal{C}$-space, 
...
\begin{figure}[!ht]
  \centering 
      \includegraphics[width=0.9\textwidth]{figChap3/InformedRRTstar_elips.png}
...
For instance, in a 2D $\mathcal{C}$-space, as depicted in Figure \ref{fig:GKDEmaze}, 
...
An illustrative example of kernel estimation in the $\mathcal{C}$-space depicted 
...
a third one for its rotation angle, denoted as $q=(x,y,\alpha)$. 
...
%---------------------------------------------------------------------------------
\\[12pt]
To extend the improved RRT* algorithm to a 6D $\mathcal{C}$-space, 
such as in the Piano Movers Problem, 
similar procedures to those in 3D $\mathcal{C}$-space 
need to be followed. 
However, 
now six coordinates are required to describe the configuration of the robot, 
denoted as $q=(x,y,z,yaw,roll,pitch)$. 
Additionally, the robot will be represented by a 3D triangle mesh.
With the adoption of a 3D triangle mesh representation, 
collision detection becomes more complex. 
To address this challenge,
the RAPID library~\cite{gottschalk1997rapid} can be utilized. 
...
It provides a narrow and user-friendly API (Application Programming Interface) 
for programmers, 
simplifying the detection of intersections between objects 
in various applications such as physically based modeling, 
...
Figure \ref{fig:6Dconfig-space} shows an example of a 3D workspace 
with a corresponding 6D $\mathcal{C}$-space.
%---------------------------------------------------------------------------------
%---------------------------------------------------------------------------------
\begin{figure}[!ht]
  \centering 
  ...
  \begin{subfigure}[b]{0.49\textwidth}
    ...
    \caption{Improved RRT* front view.}
    \label{fig:rrtstarML3D_maze_right}
  \end{subfigure}
  \begin{subfigure}[b]{0.49\textwidth}
    \includegraphics[width=\textwidth]{figChap4/6D_RRTstarML193it_loc_135to10.pdf}
    \caption{Improved RRT* left view.}
    \label{fig:rrtstarML3D_maze_top}
\end{subfigure}
  \caption{An example of path planning in the 3D workspace 
  with the 6D $\mathcal{C}$-space.
  ...
  It can be seen that improved RRT* demonstrates better performance 
  than RRT* by finding a shorter path to the goal from the same number of iterations.}
...
In addition to testing the runtime of the code, 
the experiments were conducted on a computer with 
the specifications detailed in Table \ref{tab:PC_params}  
This information is provided to ensure transparency of the results.
%---------------------------------------------------------------------------------
\begin{table}[!ht]
  \centering
...
\end{table}
%---------------------------------------------------------------------------------
\section{2D Configuration Space}
...
%---------------------------------------------------------------------------------
\\
Additionally, all OMPL planners start with a rewiring factor of $k=1.1$. 
...
the rewiring radius is set to $r = 40$ and the step size to $s = 10$. 
...
In all maps discussed in this subsection, 
the robot is represented by a point,
with its configuration denoted by two coordiniates $q=(x,y)$,
indicatind a 2D $\mathcal{C}$-space. 
A green circle indicates the $q_\text{start}$, 
a red ring signifies the $\mathcal{C}_\text{goal}$, 
and a red point denotes the $q_\text{goal}$. 
Black areas represent the $\mathcal{C}_\text{obs}$.
%---------------------------------------------------------------------------------
\begin{figure}[!ht]
  \centering 
  \begin{subfigure}[b]{0.48\textwidth}
  ...
  \end{subfigure}  
  \caption{Map \ref{fig:maze_U_Cspace} and 
...
depict the maps and convergence graphs of each planner. 
...
\\[12pt]
In Figure \ref{fig:maze_U_Cspace}, 
a simple map, previously discussed, is shown. 
The implemented RRT*\_ML 
...
\begin{figure}[!ht]
  ...
  \begin{subfigure}[b]{0.49\textwidth}
      ...
      \label{fig:maze_E_Cspace} 
  \end{subfigure}   
  \caption{Map with narrow passages \ref{fig:maze_E_Cspace} and 
...
\begin{figure}[!ht]
  ...
  \end{subfigure}   
  \caption{Cluttered map \ref{fig:maze_clutter_Cspace} and 
  corresponding convergence graphs \ref{fig:maze_clutter_graphs} of selected planners.}
  \label{fig:maze_clutter}
\end{figure}
\\
Figure \ref{fig:maze_E_Cspace} illustrates a simple example of a map with narrow passages. 
...
Figure \ref{fig:maze_clutter_Cspace} presents a cluttered map where all methods exhibit similar behavior. 
However, Improved RRT* initiates convergence to the first possible solution earlier than the others and 
demonstrates better convergence to the optimal solution than RRT*. 
In Figure \ref{fig:maze_narrow_Cspace}, 
...
\begin{figure}
  ...
  \end{subfigure}  
  \caption{Difficult map with numerouse narrow passages \ref{fig:maze_narrow_Cspace} 
  and corresponding convergence graphs \ref{fig:maze_narrow_graphs} of selected planners.}
  \label{fig:maze_narrow}
\end{figure}
%---------------------------------------------------------------------------------
\\[12pt]
Figure \ref{fig:learning_clutter} demonstrates how 
the RRT*\_ML algorithm learns and adapts to the cluttered map. 
Using the example of a cluttered map in Figure \ref{fig:maze_clutter_Cspace}, 
...
The cluttered map was selected for its potential to offer a greater diversity 
of paths compared to other maps due to the multitude of possible path variations.
%---------------------------------------------------------------------------------
\begin{figure}
  \centering
  ...
  \begin{subfigure}[b]{0.32\textwidth}
    ...
    % \label{fig:maze_E_graphs} 
  \end{subfigure}  
  \caption{Learning $\mathcal{C}$-space in the cluttered map \ref{fig:maze_clutter_Cspace}.}
...
\begin{figure}[!ht]
  ...
  in the cluttered map \ref{fig:maze_clutter_Cspace}.}
...
\begin{figure}[!ht]
  ...
  \caption{Time taken to learn predicting points (Algorithm \ref{alg:ml_sample}) in the
  cluttered map \ref{fig:maze_clutter_Cspace} to be in the \(C_{\text{free}}\).}
...
The average time spent by RRT*\_ML to predict point (Algorithm \ref{alg:ml_sample}) in the $\mathcal{C}_\text{fere}$ 
...
the $\mathcal{X}_{\text{free}}$ dataset contains 4816 datapoints, 
and the $\mathcal{X}_{\text{obs}}$ dataset contains 1954 datapoints.
%---------------------------------------------------------------------------------
...
\end{subfigure}    
  \caption{In Figure \ref{fig:maze_E_2D_Cspace}, 2D workspace and 
  robot are depicted, 
  while Figure \ref{fig:maze_E_2D_solution} shows the found solution.
...
%---------------------------------------------------------------------------------
\\
All parameters in 3D $\mathcal{C}$-space are the same as in the 2D $\mathcal{C}$-space, 
but now the robot is represented as an 'L'-shaped polygon, 
with its configuration denoted by $q=(x,y,\alpha)$. 
...
the computational time for prediction ((Algorithm \ref{alg:ml_sample})) 
increases with each iteration. 
As a result, RRT* is significantly faster, 
completing 10000 iterations in $\smallsim$30 seconds, while RRT*\_ML took $\smallsim$1041 seconds 
for the same number of iterations.
After 10000 iterations, 
the $\mathcal{X}_{\text{free}}$ dataset contains 6551 datapoints, 
and the $\mathcal{X}_{\text{obs}}$ dataset contains 19246 datapoints.
%---------------------------------------------------------------------------------
\begin{figure}[!ht]
  \centering 
  \begin{subfigure}[t]{0.49\textwidth}
  ...
  \begin{subfigure}[t]{0.49\textwidth}
    ...
    \caption{Time taken to learn predicting points (Algorithm \ref{alg:ml_sample}) 
    in the 2D workspace with the 3D $\mathcal{C}$-space (\ref{fig:maze_E_2D_Cspace})
    to be in the \(C_{\text{free}}\).}
    \label{fig:graph_learning_time_2D}
  \end{subfigure} 
  \caption{Convergence and time graphs for the 3D $\mathcal{C}$-space.}
  \label{fig:graphs_2D}
\end{figure}
%---------------------------------------------------------------------------------
\\
Computation in 6D $\mathcal{C}$-space is significantly more complex. 
...
The configuration of the robot is denoted by $q=(x,y,z,yaw,roll,pitch)$.
The cost is computed as the Euclidean distance, 
with the reference point of the robot located at its center of mass. 
This map is 25x25x25, so the $s$ was changed to 3 and the $r$ to 12. 
The $p_{goal}$ remains at 0.05. 
\\[12pt]
...
The runtime for RRT*\_ML over 200 iterations is ${\smallsim}504$ seconds,
while the runtime for RRT* is $\smallsim$379 seconds. 
...
This increase is due to the \( \mathcal{X}_{\text{free}} \) 
and \( \mathcal{X}_{\text{obs}} \) datasets accumulating more data points over time, 
making them larger.
After 200 iterations, 
the $\mathcal{X}_{\text{free}}$ dataset contains 440 datapoints, 
and the $\mathcal{X}_{\text{obs}}$ dataset contains 46 datapoints.
...
\begin{figure}[!ht]
  ...
  \begin{subfigure}[t]{0.49\textwidth}
    ...
    \caption{Convergence graph of RRT*\_ML and RRT* in 
    the 3D workspace with the 6D $\mathcal{C}$-space.}
    \label{fig:graph_6D}
  \end{subfigure}
  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\textwidth]{figChap5/graph_6D_learning.pdf}   
    \caption{Time taken to learn predicting points (Algorithm \ref{alg:ml_sample}) 
    in the 3D workspace with the 6D $\mathcal{C}$-space (\ref{fig:6Dconfig-space_chap5})
    to be in the \(C_{\text{free}}\).}
    \label{fig:graph_learning_time_6D}
  \end{subfigure} 
  \caption{Convergence and time graphs for the 6D $\mathcal{C}$-space.}
  % \label{fig:graphs_2D}
\end{figure}
%---------------------------------------------------------------------------------
%---------------------------------------------------------------------------------
\begin{figure}[!ht]
  ...
  \begin{subfigure}[b]{0.49\textwidth}
    ...
    % \label{fig:rrtstarML3D_maze_top}
\end{subfigure}
  \caption{An example of a 3D workspace with a 6D $\mathcal{C}$-space, 
  showing the found solution path. 
  The green color represents obstacle object. 
  The purple color indicates object representing our robot. 
  The red line shows the found solution path, 
  and the blue lines illustrate the paths explored in the $\mathcal{C}$-space.}
...
\section{Result}
As a result, 
it can be concluded that the implemented RRT*\_ML algorithm utilizing machine learning methods 
...
than other planners. 
However, in challenging scenarios, such as in Figure \ref{fig:maze_narrow_Cspace}, 
it behaves similarly to RRT*. 
In other instances, 
it achieves either equivalent or superior convergence to the optimal solution compared to RRT*.
However, implementation in Python and computationally intensive function
to predicting points (Algorithm \ref{alg:ml_sample}), causing RRT*\_ML to work much slower
than other planners.
%---------------------------------------------------------------------------------
%---------------------------------------------------------------------------------
%---------------------------------------------------------------------------------
...
The enhanced version often discovers the first possible solution earlier 
...
In challenging scenarios, such as those demonstrated in Figure \ref{fig:maze_narrow_Cspace}, 
the enhanced RRT* behaves similarly to traditional RRT*. 
Additionally, the implemented algorithm runs significantly slower due to its 
implementation in Python and the method used to predict points 
in $\mathcal{C_\text{free}}$ space (Algorithm \ref{alg:ml_sample}). 
Although this method is crucial for our implementation, 
it takes considerable time to predict points, 
and as the $\mathcal{C}$-space is learned, 
more data points are needed to store. 
However, the main objective is to implement and 
assess the performance of the method
in terms of convergence to the optimal solution through iterations 
and this goal is reached.
\\[12pt]
Overall, 
...
Additionally, 
implementing the algorithm in C++ could help reduce runtime and improve efficiency.
%---------------------------------------------------------------------------------
%---------------------------------------------------------------------------------
%---------------------------------------------------------------------------------
...
\appendix
\chapter*{Attachments}
The attached file \texttt{code\_Bc.zip} contains implemented methods for 2D, 
3D, and 6D configuration spaces, along with object files, graph and path visualization codes. 
Additionally, it includes C++ code to run planners from the OMPL library. 
The file also contains mazes for 2D workspace, 
represented as a set of vertices to define polygons. 
Furthermore, it includes a Makefile to facilitate running the C++ code.
\\[12pt]
The attached file \texttt{latex\_Bc.zip} contains LaTeX code of this thesis 
along with figures that were used in this thesis.
